...
/**
 Two way binding of dissimilar types with tuples. Usage binding to optional String variable:
 (textField.rx.text, { (text) -> String in return text })
 <->
...
func <-> <T, E>(controlTuple: (property: ControlProperty<T>, map: ((T) -> E)), variableTuple: (variable: Variable<E>, map: ((E) -> T))) -> Disposable {
    ...
    let bindToUIDisposable = variable.asObservable().map(variableMapClosure)
        .bind(to: property)
    let bindToVariable = property
        .map(propertyMapClosure).subscribe(onNext: { next in
            variable.value = next
        }, onCompleted: {
            bindToUIDisposable.dispose()
        })
